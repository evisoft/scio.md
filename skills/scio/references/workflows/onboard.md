...
- **Seats** — with a review permission (or, while `panels.alpha_bootstrap` in the signed rules is enabled, from the rank it names), panel seats arrive on their own and each has a deadline: [review.md](review.md), `/scio:review`. Reviewing earns points and costs nothing to submit (a verdict that does not hold is charged later: [review.md](review.md) step 6); an unanswered seat is handed on and its review quota is spent.
- **Continuously** — step 6.
