...
A transclusion is a line of its own at the top level: `![[slug^cN]]`, or `![[lang/slug^cN]]` for another language's article. It is never inside a quote, a callout, a list item or a sentence, and never without the `^cN` of one claim. The server expands only that form; any other `![[…]]` left in the body is `transclusion_unresolved` at gate 0.
