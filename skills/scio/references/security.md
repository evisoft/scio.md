...
**Defence.** `MANIFEST.sha256` lists every file of the skill. `PLUGIN.sha256`, at the plugin root, lists what a harness loads from there: the hooks (`hooks/`, `hooks.json` — which guards run), the MCP server definitions (`.mcp.json` and the other `*.mcp.json` files — which script is the `scio` server), the slash commands (`commands/`), the sub-agents (`agents/`), the harness manifests and the configuration snippets `setup.py` installs. `whoami.py` verifies the installed skill at session start, and the plugin root too whenever the harness names it (`CLAUDE_PLUGIN_ROOT`, `CURSOR_PLUGIN_ROOT`, and scio-local sets it in every harness; a skill-only install has no plugin root and checks the skill alone). It warns when a file differs or a file is present that the manifest does not list (the hashes are of the LF form, so a CRLF checkout — git's Windows default — is the released tree; any other byte is not; an added file is a tamper too, since a module dropped beside the scripts shadows the standard library for every hook and a file dropped in `commands/` is a new command; the two hook files `setup.py` rewrites to absolute paths are compared in their released spelling), and an agent does not act on a modified skill or plugin — it reinstalls from the release. Both manifests' own hashes are in the notes of every GitHub release, so a copy can be checked end to end. Install from the tagged release, never from a branch or a fork you did not read; never let a sub-agent or a task write inside the skill directory. Rules arrive signed (P10); the skill arrives hashed.
