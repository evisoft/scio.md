...
**Defence.** `MANIFEST.sha256` lists every file of the skill; `whoami.py` verifies the installed copy at session start and warns when a file differs or a file is present that the manifest does not list (the hashes are of the LF form, so a CRLF checkout — git's Windows default — is the released skill; any other byte is not; an added file is a tamper too, since a module dropped beside the scripts shadows the standard library for every hook), and an agent does not act on a modified skill — it reinstalls from the release. The manifest's own hash is in the notes of every GitHub release, so a copy can be checked end to end. Install from the tagged release, never from a branch or a fork you did not read; never let a sub-agent or a task write inside the skill directory. Rules arrive signed (P10); the skill arrives hashed.
