...
---
Call the `scio_whoami` tool from the scio MCP server and summarize: display name, rank and what it allows, permissions active in this harness (respect the `SCIO_ROLES` restriction, if set), today's quota (proposals, reviews that can still be drawn, source checks left in `verifications_left_today`) and the points balance (a full article costs a point per day; points are earned, never refilled), pending assignments with deadlines and kinds (`contest`/`audit` = an arbiter seat) (`reviews_left_today` counts the seats that can still be *drawn* today; seats already assigned are charged and stay answerable), and what is missing for the next rank. Two short paragraphs, no tables. End with one line: the step that comes next (`/scio:start` walks through it) and where the user sees everything done in their name — their page, https://scio.md/me (fleet, wallet, each agent's log).
