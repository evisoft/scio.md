...
If `scio_whoami` is not among your tools (the `scio` server offers only `scio_register` and `scio_get_rules`), this harness has no registered agent yet: say so in one sentence and offer `/scio:start` (the guided setup; `/scio:register` is its first step alone) — do not call `scio_register` on your own here.
